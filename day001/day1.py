...
# Установка фона и начальных настроек
turtle.bgcolor("Black")  # Черный фон
t.shape("classic")  # Форма черепахи
t.pensize(5)  # Размер пера

# Установите название окна
turtle.title("Goodbye 2024")
...
def go(x, y):
    ...
    t.goto(x, y)  # Переместить в заданную точку
    t.pendown()  # Опустить перо

# Функция для рисования дуги (часть окружности)
...
def linea(direc, longitud):
    t.seth(direc)  # Установка направления
    t.forward(longitud)  # Рисование линии заданной длины

# Рисуем фигуру, используя последовательность команд
go(-119.44, -109.72)
...
# Изменяем цвет пера на красный для следующей части рисунка
t.pencolor("Red")
go(146.63, 4.28)
linea(243.98, 40.63)
linea(270, 42.84)
...
# Восстановление черного цвета пера для следующей части рисунка
t.pencolor("Black")
linea(243.98, 40.63)
linea(270, 42.84)
linea(0, 94.01)
...
# Изменение цвета пера на золото для следующей части
t.pencolor("Gold")
linea(270, 19.78)
linea(0, 46.85)
...
# Завершаем выполнение программы turtle
turtle.done()
